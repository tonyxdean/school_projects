...
void StudentTest()
{
	...
	std::cout << "Additional quarter testing"<<std::endl;
	Matrix* ma1 = NULL;
	ma1 = m4.quarter();
	assert(ma1 != NULL);
	for(unsigned int i=0;i<4;i++)
	...
	std::cout<<"testing for the get col function"<<std::endl;
	double* col=NULL;
	col = m1.get_col(1);
	//test the get col function
	for(unsigned int i=0;i< 4;i++)
	{
		m1.get(i,1,temp);
		assert(col[i]==temp);
	...
	std::cout<<"test the get row function"<<std::endl;
	col=m1.get_row(1);
	for(unsigned int i=0;i< 4;i++)
	{
		m1.get(1,i,temp);
	...
	//test the inequality on an object compared to itself
	assert(!(m1!=m1));
